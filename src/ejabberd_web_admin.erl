...
is_acl_match(Host, Rules, Jid) ->
    %?INFO_MSG("======dungang=====JId , Rules ~s, ~s",[Jid,Rules]),
    lists:any(fun (Rule) ->
...
get_auth_account(HostOfRule, AccessRule, User, Server,
    ...
    %% @author dungang
    %% @date 2014-8-22
    %% @description Pass => jlib:md5(Pass) . jlib:md5 added by dungang
    case ejabberd_auth:check_password(User, Server, md5_string:md5_hex(Pass)) of
      {true,UserName } ->
        	  case is_acl_match(HostOfRule, AccessRule,
        			    jlib:make_jid(UserName, Server, <<"">>))
        	      of
        	    false -> {unauthorized, <<"unprivileged-account">>};
        	    true -> {ok, {UserName, Server}}
        	  end;
      false ->
...
process_admin(Host,
    ...
    make_xhtml([?XCT(<<"h1">>, <<"Statistics">>)] ++ Res,
	       Host, Lang, AJID);
process_admin(Host,
	      #request{path = [<<"user">>, UID],
		       auth = {_, _Auth, AJID}, q = Query, lang = Lang}) ->
	?INFO_MSG("<<<<<<<<<<<<< user = ~s",[ejabberd_auth:is_user_exists(UID, Host) ]),
    case ejabberd_auth:is_user_exists(UID, Host) of
      true ->
	  	Res = user_info(UID, Host, Query, Lang),
