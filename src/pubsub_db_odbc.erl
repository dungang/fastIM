...
read_subscription(SubID) ->
    case
      ejabberd_odbc:sql_query_t([<<"select opt_name, opt_value from im_pubsub_subscr"
...
%% -------------- Internal utilities -----------------------
    ejabberd_odbc:sql_query_t([<<"delete from im_pubsub_subscription_opt "
				 "where subid = '">>,
			       ejabberd_odbc:escape(SubID), <<"'">>]),
    ok.
...
add_subscription(#pubsub_subscription{subid = SubId,
    ...
    lists:foreach(fun (Opt) ->
			  ...
			  ejabberd_odbc:sql_query_t([<<"insert into im_pubsub_subscription_opt(subid, "
