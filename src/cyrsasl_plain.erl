...
mech_step(State, ClientIn) ->
    case prepare(ClientIn) of
	  ...
	  case (State#state.check_password)(User, Password) of
	    {true, AuthModule,Newusername} ->
        		{ok,
        		 [{username, Newusername}, {authzid, AuthzId},
...
prepare(ClientIn) ->
    ...
    case parse(ClientIn) of
      [<<"">>, UserMaybeDomain, Password] ->
          ...
          %%case parse_domain_from_string(UserMaybeDomain) of
        	    %% <NUL>login@domain<NUL>pwd
        	    [Account, _Domain] -> [UserMaybeDomain, Account, Password];
        	    %% <NUL>login<NUL>pwd
        	    [Account] -> [<<"">>, Account, Password]
        	  end;
          %% login@domain<NUL>login<NUL>pwd
          [AuthzId, Account, Password] -> [AuthzId, Account, Password];
      _ -> error
    end.
...
%% @author dungang
%% @date 2014-8-22
%% parse_domain_from_string(String)->
%%     List = string:tokens(String,"@"),
%%     Domain = lists:last(List),
%%     Account = string:join(lists:delete(Domain,List),"@"),
%%     [Account,Domain].
