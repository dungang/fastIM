...
store_room(LServer, Host, Name, Opts, odbc) ->
    ...
    F = fun () ->
		odbc_queries:update_t(<<"im_muc_room">>,
				      [<<"name">>, <<"host">>, <<"opts">>],
				      [SName, SHost, SOpts],
				      [<<"name='">>, SName, <<"' and host='">>,
				       SHost, <<"'">>])
...
restore_room(LServer, Host, Name, odbc) ->
    ...
    case catch ejabberd_odbc:sql_query(LServer,
				       [<<"select opts from im_muc_room where name='">>,
...
forget_room(LServer, Host, Name, odbc) ->
    ...
    SHost = ejabberd_odbc:escape(Host),
    F = fun () ->
		ejabberd_odbc:sql_query_t([<<"delete from im_muc_room where name='">>,
					   SName, <<"' and host='">>, SHost,
					   <<"';">>])
	end,
    ejabberd_odbc:sql_transaction(LServer, F).
...
get_rooms(LServer, Host, odbc) ->
    SHost = ejabberd_odbc:escape(Host),
    case catch ejabberd_odbc:sql_query(LServer,
				       [<<"select name, opts from im_muc_room ">>,
					<<"where host='">>, SHost, <<"';">>])
	of
...
get_nick(LServer, Host, From, odbc) ->
    ...
    SHost = ejabberd_odbc:escape(Host),
    case catch ejabberd_odbc:sql_query(LServer,
				       [<<"select nick from im_muc_registered where "
					  "jid='">>,
					SJID, <<"' and host='">>, SHost,
					<<"';">>])
	of
...
set_nick(LServer, Host, From, Nick, odbc) ->
    ...
    F = fun () ->
		case Nick of
		  <<"">> ->
		      ejabberd_odbc:sql_query_t([<<"delete from im_muc_registered where ">>,
						 <<"jid='">>, SJID,
						 <<"' and host='">>, Host,
						 <<"';">>]),
		      ok;
		  _ ->
		      Allow = case
				ejabberd_odbc:sql_query_t([<<"select jid from im_muc_registered ">>,
							   <<"where nick='">>,
							   SNick,
							   <<"' and host='">>,
							   SHost, <<"';">>])
		      ...
		      if Allow ->
			     odbc_queries:update_t(<<"im_muc_registered">>,
						   [<<"jid">>, <<"host">>,
						    <<"nick">>],
						   [SJID, SHost, SNick],
						   [<<"jid='">>, SJID,
      ...
      fun(Host, #muc_room{name_host = {Name, RoomHost}, opts = Opts}) ->
              case str:suffix(Host, RoomHost) of
                  true ->
                      ...
                      SOpts = ejabberd_odbc:encode_term(Opts),
                      [[<<"delete from im_muc_room where name='">>, SName,
                        <<"' and host='">>, SRoomHost, <<"';">>],
                       [<<"insert into im_muc_room(name, host, opts) ",
                          "values (">>,
                        <<"'">>, SName, <<"', '">>, SRoomHost,
                        <<"', '">>, SOpts, <<"');">>]];
                  false ->
      ...
      fun(Host, #muc_registered{us_host = {{U, S}, RoomHost},
              ...
              case str:suffix(Host, RoomHost) of
                  true ->
                      ...
                      SNick = ejabberd_odbc:escape(Nick),
                      SRoomHost = ejabberd_odbc:escape(RoomHost),
                      [[<<"delete from im_muc_registered where jid='">>,
                        SJID, <<"' and host='">>, SRoomHost, <<"';">>],
                       [<<"insert into im_muc_registered(jid, host, "
                          "nick) values ('">>,
