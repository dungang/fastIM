...
create_node(NodeIdx, Owner) ->
	...
	?INFO_MSG("======  create node sql ====== ~s",[<<"insert into im_pubsub_state(nodeid, jid, "
				   "affiliation, subscriptions) values(">>,
				 state_to_raw(NodeIdx, State), <<");">>]),
    catch
      ejabberd_odbc:sql_query_t([<<"insert into im_pubsub_state(nodeid, jid, "
				   "affiliation, subscriptions) values(">>,
				 state_to_raw(NodeIdx, State), <<");">>]),
    {result, {default, broadcast}}.
...
delete_node(Removed) ->
    Reply = lists:map(fun (#pubsub_node{id = NodeId} =
			      ...
			      Subscriptions = case catch
						     ejabberd_odbc:sql_query_t([<<"select jid, subscriptions from im_pubsub_state "
										  "where nodeid='">>,
										NodeId,
										<<"';">>])
						  of
...
get_entity_affiliations(Host, Owner) ->
    ...
    Reply = case catch
		   ejabberd_odbc:sql_query_t([<<"select node, type, i.nodeid, affiliation "
						"from im_pubsub_state i, pubsub_node n where "
...
get_node_affiliations(NodeIdx) ->
    Reply = case catch
		   ejabberd_odbc:sql_query_t([<<"select jid, affiliation from im_pubsub_state "
						"where nodeid='">>,
					      NodeIdx, <<"';">>])
		of
...
get_affiliation(NodeIdx, Owner) ->
    ...
    J = encode_jid(GenKey),
    Reply = case catch
		   ejabberd_odbc:sql_query_t([<<"select affiliation from im_pubsub_state "
						"where nodeid='">>,
					      NodeIdx, <<"' and jid='">>, J,
					      <<"';">>])
		of
...
get_entity_subscriptions(Host, Owner) ->
    ...
    Query = case SubKey of
		  ...
		  [<<"select node, type, i.nodeid, jid, subscriptio"
		     "ns from im_pubsub_state i, im_pubsub_node "
...
get_entity_subscriptions_for_send_last(Host, Owner) ->
    ...
    Query = case SubKey of
		  ...
		  [<<"select node, type, i.nodeid, jid, subscriptio"
		     "ns from im_pubsub_state i, im_pubsub_node "
		     "n, im_pubsub_node_option o where i.nodeid "
...
get_subscriptions(NodeIdx, Owner) ->
    ...
    J = encode_jid(SubKey),
    Reply = case catch
		   ejabberd_odbc:sql_query_t([<<"select subscriptions from im_pubsub_state "
						"where nodeid='">>,
					      NodeIdx, <<"' and jid='">>, J,
					      <<"';">>])
		of
...
get_state_without_itemids(NodeIdx, JID) ->
    ...
    case catch
	   ejabberd_odbc:sql_query_t([<<"select jid, affiliation, subscriptions "
					"from im_pubsub_state where jid='">>,
...
set_state(NodeIdx, State) ->
    ...
    case catch
	   ejabberd_odbc:sql_query_t([<<"update im_pubsub_state set subscriptions='">>,
				      S, <<"', affiliation='">>, A,
...
del_state(NodeId, JID) ->
    J = encode_jid(JID),
      ...
      ejabberd_odbc:sql_query_t([<<"delete from im_pubsub_state where jid='">>,
				 J, <<"' and nodeid='">>, NodeId, <<"';">>]),
    ok.
...
get_items(NodeId, _From) ->
    case catch
	   ejabberd_odbc:sql_query_t([<<"select itemid, publisher, creation, "
					"modification, payload from im_pubsub_item "
					"where nodeid='">>,
				      NodeId,
...
get_items(NodeId, From, none) ->
    MaxItems = case catch
		      ejabberd_odbc:sql_query_t([<<"select val from im_pubsub_node_option where "
						   "nodeid='">>,
						 NodeId,
						 <<"' and name='max_items';">>])
		   of
...
get_items(NodeId, _From,
    ...
    [AttrName, Id] = case I of
			   ...
			   case catch
				  ejabberd_odbc:sql_query_t([<<"select modification from im_pubsub_item "
							       "pi where exists ( select count(*) as "
							       "count1 from im_pubsub_item where nodeid='">>,
							     NodeId,
    ...
    Count = case catch
		   ejabberd_odbc:sql_query_t([<<"select count(*) from im_pubsub_item where "
						"nodeid='">>,
					      NodeId, <<"';">>])
		of
	      {selected, [_], [[C]]} -> C;
...
set_item(Item) ->
    ...
    case catch
	   ejabberd_odbc:sql_query_t([<<"update im_pubsub_item set publisher='">>,
				      P, <<"', modification='">>, S(M),
				      <<"', payload='">>, XML,
				      <<"' where nodeid='">>, NodeId,
				      <<"' and itemid='">>, I, <<"';">>])
	of
      ...
      _ ->
	    ...
	    ejabberd_odbc:sql_query_t([<<"insert into im_pubsub_item (nodeid, itemid, "
...
del_item(NodeId, ItemId) ->
    I = (?PUBSUB):escape(ItemId),
      ...
      ejabberd_odbc:sql_query_t([<<"delete from im_pubsub_item where itemid='">>,
				 I, <<"' and nodeid='">>, NodeId, <<"';">>]).
...
itemids(NodeId, SJID) ->
    case catch
	   ejabberd_odbc:sql_query_t([<<"select itemid from im_pubsub_item where "
					"nodeid='">>,
...
update_affiliation(NodeId, JID, Affiliation) ->
    ...
    case catch
	   ejabberd_odbc:sql_query_t([<<"update im_pubsub_state set affiliation='">>,
