...
delete_node(Host, Node) ->
    ...
    Removed = get_subnodes_tree(Host, Node),
      ...
      ejabberd_odbc:sql_query_t([<<"delete from im_pubsub_node where host='">>,
				 H, <<"' and node like '">>, N, <<"%';">>]),
    Removed.
...
raw_to_node(Host, [Node, Parent, Type, NodeIdx]) ->
    Options = case catch
		     ejabberd_odbc:sql_query_t([<<"select name,val from im_pubsub_node_option "
						  "where nodeid='">>,
						NodeIdx, <<"';">>])
		  of
...
set_node(Record) ->
    ...
    NodeIdx = case nodeid(Host, Node) of
	       {result, OldNodeIdx} ->
		     ...
		     ejabberd_odbc:sql_query_t([<<"delete from im_pubsub_node_option where "
						  "nodeid='">>,
						OldNodeIdx, <<"';">>]),
		   catch
		     ejabberd_odbc:sql_query_t([<<"update im_pubsub_node set host='">>,
						H, <<"' node='">>, N,
						<<"' parent='">>, P,
						<<"' type='">>, Type,
						<<"' where nodeid='">>,
	       ...
	       _ ->
		     ...
		     ejabberd_odbc:sql_query_t([<<"insert into im_pubsub_node(host, node, "
						  "parent, type) values('">>,
						H, <<"', '">>, N, <<"', '">>, P,
						<<"', '">>, Type, <<"');">>]),
		   case nodeid(Host, Node) of
    ...
    case NodeIdx of
	  ...
	  lists:foreach(fun ({Key, Value}) ->
				...
				SValue =
				  ...
				  ejabberd_odbc:sql_query_t([<<"insert into im_pubsub_node_option(nodeid, "
							       "name, val) values('">>,
							     NodeIdx, <<"', '">>,
							     SKey, <<"', '">>,
							     SValue, <<"');">>])
...
nodeid(Host, NodeId) ->
    ...
    case catch
	   ejabberd_odbc:sql_query_t([<<"select nodeid from im_pubsub_node where "
					"host='">>,
				      H, <<"' and node='">>, N, <<"';">>])
	of
