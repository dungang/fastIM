...
check_password(User, Server, Password) ->
    case check_password_with_authmodule(User, Server,
					Password)
	of
      %%由原来的{true, _AuthModule} -> true;
      %%改写成 {true, _AuthModule,UserName} -> {true, UserName};
      %%UserName 作为唯一标示，用户的Id
      %% @author dungang
      %% @date 2014-8-22
      {true, _AuthModule,UserID} -> {true, UserID};
      false -> false
    end.
...
%%       %% @author dungang
%%       %% @date 2014-8-22
%%       {true, _AuthModule,UserName} -> {true, UserName};
%%       false -> false
%%    end.
...
check_password_loop([AuthModule | AuthModules], Args) ->
    case apply(AuthModule, check_password, Args) of
      %%由原来的true -> {true, AuthModule};
      %%改写成 {true,UserName} -> {true, AuthModule,UserName};
      %%UserName 作为唯一标示，用户的Id
      %% @author dungang
      %% @date 2014-8-22
      {true,UserName} -> {true, AuthModule,UserName};
